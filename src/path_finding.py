...
def main(win, width):
	...
	while run:
		...
		for event in pygame.event.get():
			...
			if pygame.mouse.get_pressed()[0]: # LEFT: add a node 
			...
			elif pygame.mouse.get_pressed()[2]: # RIGHT: erase a node 
				pos = pygame.mouse.get_pos()
				row, col = get_clicked_pos(pos, ROWS, width)
				spot = grid[row][col]
				spot.reset()
			...
			if event.type == pygame.KEYDOWN: # SPACE: start the game 
				...
				# 1: astar, 2: dijkstra, 3: dfs, 4: bfs
